...
class TelegramMyfinEgsBotController extends Controller
{
    ...
    {
        Log::info('Handle method called'); // Metod ishga tushganini log qilish
        try {
            // Telegramdan kelgan yangilanishlarni olish
            $update = $this->telegram->getWebhookUpdate();
            Log::info('Update received: ' . json_encode($update)); // Yangilanish logi

            // Agar yangilanishda xabar bo‘lsa
            if ($update->getMessage()) {
                $message = $update->getMessage();
                $chatId = $message->getChat()->getId();
                Log::info('Message received from chat ID: ' . $chatId); // Xabar kelgan chat ID

                // Faqat ruxsat berilgan foydalanuvchilardan xabar qabul qilish
